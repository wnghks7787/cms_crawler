...
# def download_js_files(url, output_dir="js_files"):
#     """
#     주어진 URL의 HTML에서 발견된 모든 JavaScript 파일을 다운로드합니다.
#     302 리디렉션을 자동으로 처리하도록 requests를 사용합니다.
#     """
#     try:
#         # 출력 디렉토리가 없으면 생성
...
#         # 웹 페이지 소스코드 가져오기 (리디렉션 자동 처리)
#         # allow_redirects=True가 기본값이지만 명시적으로 설정
#         response = requests.get(url, allow_redirects=True)
        
#         # HTTP 오류 발생 시 예외 발생
#         response.raise_for_status()  
        
#         # 최종 URL 확인 (리디렉션 후의 URL)
...
#         print(f"{final_url}에서 {len(script_tags)}개의 <script> 태그를 찾았습니다.")
        
#         if not script_tags:
#             print("소스 코드에 외부 JavaScript 파일이 없습니다. 동적으로 로드되는 파일일 수 있습니다.")
        
#         for script_tag in script_tags:
#             js_url = script_tag['src']
            
#             # 상대 경로를 절대 경로로 변환 (최종 URL 기준)
#             absolute_js_url = urljoin(final_url, js_url)
            
#             # 파일 이름 추출
#             filename = os.path.basename(urlparse(absolute_js_url).path)
            
#             if not filename or '.' not in filename:
#                 # 파일 이름이 없거나 확장자가 없는 경우 고유한 이름 생성
#                 filename = f"script_{hash(absolute_js_url) % 10000}.js"
            
#             filepath = os.path.join(output_dir, filename)
            
#             print(f"다운로드 중: {absolute_js_url}")
            
#             try:
#                 js_response = requests.get(absolute_js_url, allow_redirects=True)
...
#                 print(f"다운로드 실패: {absolute_js_url}")
#                 print(f"  - 오류: {e}")
...
#     target_url = "http://localhost:10000"  # 여기에 원하는 URL을 입력하세요.
#     download_js_files(target_url)
