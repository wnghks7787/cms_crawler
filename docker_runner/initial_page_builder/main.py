...
if __name__ == "__main__":
    ...
    for REPO in REPOs:
        ...
        for i in range(0, len(pairs)):
            ...
            r = tools.run(f"docker-compose -f {path}/{shlex.quote(name)} up -d > /dev/null 2>&1 || true", check=False, capture=True)

            if r.returncode != 0:
                logger.log(f"ERROR run 실패: {name} -> {r.stderr.strip()}")

            url = f"localhost:{host_port}/"
            
            time.sleep(10)
            r = tools.run(f"./initial_page_builder.sh {repo} {host_port} {tag}", check=False)
                
            code = runner.wait_http_ready(url, timeout=30, interval=1, follow_redirects=True, max_redirs=10, treat_redirect_ok=False)
            runner.save_headers(url, os.path.join(path, "headers.txt"))
            ...
            r = tools.run(f"docker-compose -f {path}/{shlex.quote(name)} down > /dev/null 2>&1 || true", check=False, capture=True)
